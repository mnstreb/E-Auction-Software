...
window.AppHeader = (function() {
    let mainAppLogoElem;
    // The mainAppDefaultIcon span is now an img tag, so we'll rename the variable for clarity.
    let mainAppDefaultLogoElem; 
     ...
     * ... (rest of the JSDoc is the same)
     */
    function init(config) {
        mainAppLogoElem = document.getElementById(config.mainAppLogoId);
        // Get the new default logo element by its ID from the updated HTML.
        mainAppDefaultLogoElem = document.getElementById('mainAppDefaultIcon'); 
        ...
        if (exportPdfReportBtnElem) {
            exportPdfReportBtnElem.addEventListener('click', () => {
                onExportPdfReportCallback();
                hideDropdown(exportDropdownMenuElem);
            });
        }
        if (exportCsvBtnElem) {
            exportCsvBtnElem.addEventListener('click', () => {
                onExportCsvCallback();
                hideDropdown(exportDropdownMenuElem);
            });
        }
        if (exportPdfQuoteBtnElem) {
            exportPdfQuoteBtnElem.addEventListener('click', () => {
                onExportPdfQuoteCallback();
                hideDropdown(exportDropdownMenuElem);
            });
        }
        if (emailClientBtnElem) {
            emailClientBtnElem.addEventListener('click', () => {
    ...
    function updateProjectInfo(projectType, projectName, clientName, projectState) {
        if (projectTypeDisplayElem) {
            projectTypeDisplayElem.textContent = projectName;
        }
     ...
     * This function now handles showing the user's logo OR the default business logo.
     * @param {string} logoSrc - Data URL or path for the user's uploaded logo.
     */
    function updateLogo(logoSrc) {
        // This is the core updated logic.
        if (mainAppLogoElem && mainAppDefaultLogoElem) {
            if (logoSrc) {
                // If a user logo is provided, show it and hide the default logo.
                mainAppLogoElem.src = logoSrc;
                mainAppLogoElem.classList.remove('hidden');
                mainAppDefaultLogoElem.classList.add('hidden');
            } else {
                // If no user logo, hide the user logo container and show the default logo.
                mainAppLogoElem.src = '';
                mainAppLogoElem.classList.add('hidden');
                mainAppDefaultLogoElem.classList.remove('hidden');
            }
        }
    }
